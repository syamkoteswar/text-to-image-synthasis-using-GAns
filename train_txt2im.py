...
def main_train():
    ###======================== DEFIINE MODEL ===================================###
    t_real_image = tf.compat.v1.placeholder('float32', [batch_size, image_size, image_size, 3], name = 'real_image')
    t_wrong_image = tf.compat.v1.placeholder('float32', [batch_size ,image_size, image_size, 3], name = 'wrong_image')
    t_real_caption = tf.compat.v1.placeholder(dtype=tf.int64, shape=[batch_size, None], name='real_caption_input')
    t_wrong_caption = tf.compat.v1.placeholder(dtype=tf.int64, shape=[batch_size, None], name='wrong_caption_input')
    t_z = tf.compat.v1.placeholder(tf.float32, [batch_size, z_dim], name='z_noise')
    ...
    n_epoch = 11 # 600
    print_freq = 1
    n_batch_epoch = int(n_images_train / batch_size)-20
    # exit()
